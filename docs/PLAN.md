...
### 7-1. 機能切り替えUI
- [ ] タブまたはサイドバーで5機能をスムーズに切り替え
- [ ] 機能間でPDFデータを保持（再アップロード不要）

### 7-2. エラーハンドリング
- [ ] ファイル読み込みエラー（破損PDF、非PDFファイル）
- [ ] ファイルサイズ上限チェック（UI側）
- [ ] PDF処理エラー（pdf-lib の例外キャッチ）
- [ ] ユーザーへの分かりやすいエラーメッセージ表示（toast通知等）

### 7-3. 大容量ファイル対応
- [ ] Next.js の bodyParser サイズ制限の調整（API Routes使用時）
- [ ] クライアントサイド処理の場合: Web Worker でPDF処理をオフロード（UIフリーズ防止）
- [ ] 処理中のプログレスバー表示

### 7-4. UI/UXの調整
- [ ] ローディング状態の統一
- [ ] レスポンシブの基本対応（デスクトップファースト）
- [ ] アクセシビリティ基本対応（キーボード操作、aria属性）
- [ ] ダークモード対応（Tailwind の `dark:` で最低限）

### 7-5. コミット
- [ ] `feat: integrate all features and polish UI`
