...
- [x] `main` ブランチへのマージ → GitHub Actions 自動デプロイの動作確認
  - deploy.yml トリガーを `test` → `main` に変更
- [ ] デプロイ後のURL(`/PDF_Edit_Tool/`)でアクセス確認 → マージ後に手動確認
- [ ] 静的アセット（pdfjs-dist ワーカー等）の読み込み確認 → マージ後に手動確認
- [ ] basePath 設定下でのリンク整合性 → マージ後に手動確認

### 11-2. パフォーマンス計測
- [x] コード分析によるパフォーマンス特性の文書化（SESSION_SUMMARY.md）
  - プログレッシブサムネイル生成: 実装済み
  - 仮想スクロール: 未実装（v2 検討）
  - メモリ: ページ数に比例（50〜100ページは実用的）
- [ ] 大容量PDF実測 → 手動プロファイリング（Chrome DevTools）で対応

### 11-3. UX改善
- [x] タブ切り替え時のPDF保持 — 既に実装済み（確認ダイアログ表示、PDFはクリアしない）
- [x] モバイル表示の最低限の調整（ヘッダー・タブ・コンテンツのレスポンシブパディング）
- [x] キーボードショートカット検討 → Ctrl+Z Undo は状態管理の大幅変更が必要、v2 で検討

### 11-4. アーキテクチャ決定事項の文書化
- [x] API Routes を採用しない理由の記録（SESSION_SUMMARY.md に追記）
- [x] `output: "export"` 採用理由の記録（SESSION_SUMMARY.md に追記）
