...
- feat(text-rewrite): **文字の書き換えの操作画面**（T4d）を追加。編集ツールバーの「**文字を書き換え**」で書き換えモードに入り、ページ上の文字を**クリックすると語・数値のまとまり**、**ドラッグすると同じ行の文字範囲**を選べる（カーソルを乗せると候補を枠で表示）。選んだ範囲の近くに編集ボックスを開き、新しい文字・揃え（左／中央／右。数値なら右揃えが初期値）を指定して Enter または「確定」で書き換える（Esc・「キャンセル」で閉じる）。入力のたびに確定前の確認を行い、**元の書体に無い文字はゴシック体（Noto Sans JP）で描く旨**や、書き換えられない理由をその場で表示する（描けない場合は確定できない）。当たり判定・範囲・確認は保存時と同じ手順で作ったページの文字情報を使うため、選んだ範囲と実際に書き換わる範囲は一致する。書き換えは Undo/Redo でき、表示・検索・保存に反映される。編集ボックスはページ下端に近ければ選択範囲の上に出し、ビューアのスクロール領域の中だけを必要な分スクロールする。
- feat(text-rewrite): **テキストを書き換えたページの表示**（T4c）を追加。書き換え履歴のあるページは、書き換え後の 1 ページの PDF を作って pdf.js で描画し、ビューア（連続・単ページ）・サムネイル・ページ一覧整理・テキスト検索のすべてに反映する（`usePageSource`）。作成結果はページと履歴ごとにキャッシュして同時の要求をまとめ、上限を超えたら古いものから破棄する（`edited-page-cache.ts`）。同梱フォントは、元のフォントで描けない文字があったときだけ読み込んで作り直す。作成に失敗したページは元のページを表示して通知する。書き換えが無い文書では pdf-lib を読み込まない。
- feat(text-rewrite): テキストの書き換えを**編集操作ログに組み込み**（T4b）。新しい操作 `editText` でページごとの書き換え履歴（`PageRef.textEdits`）に追加し、Undo/Redo・未保存表示の対象にする。書き換えは「元の PDF 全体を読み、そのページに履歴を順に適用する」ことで再現し（`lib/editor/text-edit.ts`。他のページにある同じ書体のフォントも使える）、保存・上書き保存（Worker）・抽出・分割のすべてに反映する。確定前の確認用に、文書を変更せずに警告・失敗を返す `previewTextEdit` と、失敗理由の日本語化を追加。同梱フォントは書き換えを含む保存のときだけ取得する（`fallback-font-source.ts`、取得に失敗したら次回に取り直す）。画面の操作はまだ無い。
- feat(text-rewrite): **1 文字ずつ別の描画命令に分かれた範囲のまとめ書き換え**（T4a）を追加。同じフォント・文字サイズで同じ行に読み順どおり並んでいれば、先頭の命令に新しい文字列を入れて残りの命令の文字を削除し、範囲全体の幅を基準に左／右／中央揃えを保つ（条件を満たさない場合は従来どおり `spans-operations`）。あわせて、画面で範囲を選ぶための `text-runs.ts`（クリックで選ぶ「語・数値のまとまり」の判定、ドラッグで選ぶ同じ行の範囲、数値なら右揃え・それ以外は左揃えの初期値）を追加。
- feat(text-rewrite): テキスト書き換えで**元のフォントに無い文字を補う**処理（T3）を追加。1 文字ずつ、(1) 元フォントの ToUnicode、(2) 元フォントの埋め込み TrueType の cmap（ToUnicode と W に対応を追記して同じフォントで描く）、(3) 文書内の**同じ書体の別サブセット**（ファミリ名・グリフ総数・unitsPerEm で判定し、`Tf` を切り替えて描く）、(4) **同梱フォント Noto Sans JP**（書体が変わる旨を警告）の順で描き方を決める。同梱フォントは、@pdf-lib/fontkit の部分埋め込みでは一部の字形が表示されなかったため、**GID を保ったまま使う字形の輪郭データだけを複写する部分埋め込み**を自前で実装して組み込む（7 文字で PDF 増加 約 22KB）。書き換えが 1 件でも失敗した場合は、フォント辞書やリソースへの追記も含めて何も変更しない。画面にはまだ組み込んでいない。
- chore(fonts): 同梱フォント `public/fonts/NotoSansJP-Regular.ttf`（Noto Sans JP Regular、SIL Open Font License 1.1）とライセンス文 `NotoSansJP-OFL.txt` を追加。
- feat(text-rewrite): **同一フォントでの本物のテキスト書き換え**（T2）を追加（`src/lib/pdf/content/rewrite.ts`）。指定したグリフ範囲を新しい文字列に置き換え、描画命令の文字データそのものを書き換える（元の文字は PDF から消える）。桁数などで幅が変わっても **左／右／中央揃え**で基準位置と同じ行の後続の文字を動かさないよう TJ の間隔値で補正し、`Tj`・`'`・`"` は等価な TJ に変換する。文字幅ぴったりの矩形クリップ（Microsoft Print to PDF）やセルのクリップ（Excel）は、新しい文字がはみ出す側だけを元の文字より伸びた量だけ広げる。新しい文字は、埋め込み TrueType（`truetype.ts`）の `loca`／cmap で**字形が実際にサブセットに残っているか**を確かめてから使い、無い文字は一覧にして失敗する（文書は変更しない）。画面にはまだ組み込んでいない。
...
- feat(bookmark): **しおり編集（v2）** を追加。左ペイン「しおり」タブは**既定が閲覧モード**（クリックでジャンプのみ）で、ヘッダの「**しおり編集**」ボタンで編集モードに入る（「完了」で抜ける／ドキュメント切替で自動解除）。編集モードでは各しおりを **ドラッグで並べ替え（縦移動）＋階層変更（横移動でインデント/アウトデント）** でき、ドラッグ中はつかんだ枝を移動先から除外、横移動量から投下後の深さ・親を投影してクランプする（`@dnd-kit` のツリー投影方式）。「**現在のページを追加**」でルート末尾に現在表示ページ宛のしおりを作成、各項目の操作メニュー（⋮）から **名前を変更 / サブ項目を追加 / 上へ・下へ移動 / 階層を下げる / 階層を上げる / 削除**（キーボード操作の代替手段）も可能。子を持つ項目の削除は `AlertDialog` で確認、ダブルクリックでインライン改名。編集状態は `outline-store` が保持し、**保存時に編集後ツリーを出力 /Outlines へ書き戻す**（未編集時は従来どおり pdf.js から全ソース収集）。しおり編集は未保存状態（ステータスバー ● / `beforeunload` / 開く・閉じる確認）に反映される。
...
- feat(text-rewrite): **ドラッグで複数の文字をまとめて選べることが分かるように**表示を改善。字間を空けた見出し（「土 工 計 算 書」）はクリックだと 1 文字ずつしか選べず、ドラッグでまとめて書き換えられることに気づきにくかったため。(1) 選択範囲の色を、テーマの主色（ほぼ黒で灰色に見えていた）から一般的な文字選択と同じ**青**に変更、(2) カーソルを乗せた候補の上に「クリックで選択 ／ ドラッグでまとめて選択」を表示、(3) **ドラッグ中**は範囲を太枠・濃い青で強調し「『土 工 計 算 書』を選択中（5 文字）」と選んでいる文字を表示、(4) 1 文字などを選んだとき、同じ行のすぐ隣（文字サイズ 1 つ分以内）に別の文字があれば、編集ボックスに「隣の文字とまとめて書き換えるには、閉じてからドラッグで範囲を選択してください」と案内（`text-runs.ts` の `hasNeighborRun`。表のセルのように離れた文字では出さない）、(5) ツールバーの案内で「ドラッグで複数の文字をまとめて選択できます」を強調。
- build: `tsconfig.json` の `exclude` に `out` を追加。静的エクスポートの出力に含まれる Worker のソース（`.ts`）が次回の `next build` の型チェック対象になり、Worker のメッセージ型を変えると古い出力が原因でビルドが失敗していたため。
...
- fix(save): **大きな PDF（複数 PDF の結合など）の「名前を付けて保存」が Chrome / Edge で「保存に失敗しました」になる**問題を修正。保存は PDF を作り終えてから保存先の選択画面（`showSaveFilePicker`）を開くが、Chromium はこの画面をクリックから約 5 秒以内にしか開けず、作成に 5 秒以上かかると `SecurityError: Must be handling a user gesture to show a file picker.` で失敗していた（報告のサンプル 18 ファイル・356 ページ・268MB で作成に約 5.4 秒）。作成後に画面を開けなかった場合は、作った PDF を保持したまま「保存の準備ができました」の通知を出し、その「保存先を選ぶ」ボタン（新しいクリック）で保存先を選んで保存するようにした（`PickerActivationExpiredError`）。通知を押す前に編集した場合は保存せず、もう一度保存するよう案内する。先に保存先を選ぶ方式は、既存のファイルを選んだ時点でその中身が消える（File System Access の仕様）ため、作成に失敗したときにファイルを失うおそれがあり採用しなかった。抽出の保存も同じ扱い。
- fix(save): **分割保存**は 1 ファイルずつ保存先の選択画面を開いており、2 つ目以降が同じ理由で開けなかったため、**クリック直後に保存先のフォルダを 1 回だけ選び**（`showDirectoryPicker`、フォルダを選んでもファイルは変更されない）、作った各ファイルをそこへ書き込むようにした。同名のファイルがあれば上書きせず「名前 (1).pdf」のように別名にする。フォルダを選べないブラウザ（Firefox）は従来どおり各ファイルをダウンロードする。
...
- fix(save): 一部の PDF（例: Illustrator の「編集を制限」付きファイル）が保存できない不具合を修正。これらは**暗号化 PDF**（標準セキュリティハンドラ・ユーザーパスワードは空）で、閲覧に使う pdf.js は復号できるため表示は正常だが、保存に使う pdf-lib は復号機能を持たず `PDFDocument.load` が `Input document to PDFDocument.load is encrypted` で失敗していた（`ignoreEncryption: true` を渡しても中身が暗号文のままで解析できないため回避不可）。標準セキュリティハンドラの復号を自前実装（`src/lib/pdf/decrypt/`）し、保存前に平文 PDF へ書き直してから pdf-lib へ渡すようにした。対応方式は **V1/V2（RC4 40〜128bit）・V4（`/CFM /V2` = RC4、`/AESV2` = AES-128）・V5 R5/R6（`/AESV3` = AES-256）**。MD5・RC4・AES は Web Crypto に無い/パディング制御が必要なため自前実装し、**RFC 1321 / RFC 6229 / FIPS-197 / NIST SP 800-38A の公式テストベクタ**で検証している。出力は暗号化を解除した通常の PDF（元の権限フラグ `/P` は引き継がれない）。閲覧に実パスワードが必要な文書は復号せず、日本語で明示して失敗する。
- fix(organize): 「ページを一覧整理」で A3 横などの横長ページのサムネイルが隣と重なる不具合を修正。サムネイルは「短辺＝基準幅」でスケールしていたため、A3 横（1191×842pt）の実寸が `140×1.414 = 198px` 幅となり、グリッドのトラック幅（`thumbnailWidth + 24 = 164px`）を超えて左右へはみ出していた（既定幅で隣と約 29px 重複。サムネ幅を上げるほど悪化）。回転後のページを「幅 `thumbnailWidth` × 高さ `thumbnailWidth × 1.414`」のセルへ収めるスケール（既存の `computeFitScale` を再利用）に変更し、実寸が必ず基準幅以下になるようにした。A4 縦はセル比とほぼ一致するため従来どおりの大きさを維持する。併せてグリッド項目に `min-w-0` を付与した。
- fix(save): 保存失敗時に pdf-lib の英語メッセージがそのままトーストへ表示され、利用者が対処できない（かつ `ignoreEncryption` を使えといった内部実装の示唆を含む）問題を修正。日本語を含むメッセージ（＝アプリが意図して出したもの）のみ表示し、それ以外は定型文を出してコンソールへ詳細を残すようにした。
- fix(save): PDF 生成が失敗したとき、Worker で失敗した処理を本スレッドで再実行し、同じ例外をもう一度出したうえで UI スレッドを重い処理で塞いでいた問題を修正。フォールバックは「Worker を用意できなかった場合」（生成失敗・スクリプト読み込み失敗）に限定し、ビルド自体の失敗はそのまま伝播するようにした（`WorkerUnavailableError` で区別）。
- fix(ci): GitHub Actions（Pages デプロイ）の依存インストールを `npm ci` → `npm install` に変更し、CI Node を 20 → 22 に更新。Tailwind v4 の wasm フォールバック `@tailwindcss/oxide-wasm32-wasi` が要求する `@emnapi/core`/`@emnapi/runtime` 等のプラットフォーム依存 optional 依存を Windows 生成の `package-lock.json` に含められず、Linux ランナーの `npm ci` が同期エラーで失敗していた（npm の既知制約）。ランナー上で依存解決させて解消。
