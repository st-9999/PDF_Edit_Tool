...
- [x] しおり編集（追加・リネーム・階層変更・削除 → Outline書き戻し） — 既定は閲覧、「しおり編集」ボタンで編集モード。編集モードは**ドラッグで並べ替え（縦）＋階層変更（横）**＋⋮メニュー（追加/改名/上下/インデント/アウトデント/削除）。`outline-store`＋純関数モデル（`lib/outline/edit.ts`・`tree-dnd.ts`）。保存時に編集後ツリーを `buildPdf`/`applyOutline` で /Outlines へ書き戻し。単体: edit18＋tree-dnd12＋ストア7＋統合1。**Undo/Redo は未対応**。E2E はメニュー操作（名前の変更・現在のページの追加・階層変更・削除の確認と取消）と保存結果を追加済み（`e2e/bookmark-edit.spec.ts`。ドラッグ操作は未カバー）
- [ ] 大規模最適化の本命: **操作ログ＋遅延適用**を保存パスに統合、結合のページ単位ストリーム処理
- [x] テキスト検索の高度化（正規表現・ページ横断ナビ） — `buildMatcher` で正規表現／大小区別に対応（`findMatches`/`applyHighlights` 共通化）、検索バーにトグル＋無効式表示。可変長一致が複数ページを横断ナビ可能。単体 +14 件。E2E 追加済み（`e2e/search-options.spec.ts`: 正規表現・無効な式・大小区別）

### v3

- [x] しおり**自動生成**（方式の要件定義 → 実装）＝**報告書しおり自動作成**。本文テキストから章（第N章）/節（N.N）/項（N.N.N）の見出しを正規表現パターンで検出し最大3階層のしおりツリーを自動生成。現在の表示ページ列（結合PDF含む全体）を対象に pdf.js の `getTextContent` を再利用、フォントサイズ比例の動的許容差で行復元（`reconstructLines`）、項→節→章の順に適用＋番号後退フィルタ（`SequenceTracker`）＋TOCリーダー線除外、全角数字対応。3ステップダイアログ（設定→進捗/中止→階層プレビュー）でパターンの有効/無効・カスタム追加（localStorage永続化）・開始ページ指定が可能。既存しおりは上書き/末尾追加を選択し `outline-store.replaceTree` で反映。単体33＋実PDF統合5件。E2E 追加済み（`e2e/auto-bookmark.spec.ts`: 日本語見出しの検出と階層・保存、既存しおりの追加／上書き、開始ページ、検出 0 件。中止・カスタムパターンは未カバー）
- [ ] OCR連携（必要に応じて）
- [ ] さらなる大規模対応（インクリメンタル更新 / WASM系エンジン検討）

### テキスト書き換え（本物の書き換え）

> 設計: [`TEXT_REWRITE.md`](./TEXT_REWRITE.md)。元フォントに無い文字は同一書体の別リソース → 同梱フォントで補う（書体が変わる場合は警告）。

- [x] T0 調査: サンプル PDF のフォント種別・ToUnicode・収録グリフ・文字列の持ち方・クリップを調査し、試作で「同一フォントでの置換・右揃え補正・クリップ拡張」が成立することを確認
- [x] T1 解析: コンテンツストリームのトークナイザとテキスト解釈（グリフ単位の文字・位置・元データ位置）。`src/lib/pdf/content/`（`lexer` / `operations` / `cmap` / `font` / `text-layout` / `page-text`）。pdf.js 抽出結果（文字列・各項目の先頭と末尾の位置）との一致を生成 PDF 5 種＋実サンプル 2 件 20 ページで確認。単体・統合 73 件
- [x] T2 置換: 同一フォントでの置換、`TJ` の幅補正（右／左／中央揃え）、矩形クリップの拡張、書き戻し（ページの /Contents を新しい 1 ストリームに差し替えるため共有ストリームは不変）。`rewrite.ts`（`replacePageText` / `rewritePageContent`）＋埋め込み TrueType の字形確認（`truetype.ts`・`FontModel.encode`）。実サンプルで「81.9→123.4」「令和8→6年度」「508→123」を書き換え、他のグリフ位置が不変・pdf.js と一致することを確認。単体・統合 46 件
...
- [x] T4 UI（ユーザー決定: 範囲は「クリックでまとまり＋ドラッグで文字単位」、書体が変わる場合は確定前に編集ボックス内で表示、1 文字ずつ別命令に分かれた範囲は同じ行・フォント・サイズならまとめて書き換える）
...
- [x] 本番ビルドの E2E（サブパス配信・保存用 Worker・同梱フォント）をデプロイ前に実行（`npm run e2e:prod`・`e2e-prod/`）
