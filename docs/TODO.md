...
- [ ] `pdf.js`（react-pdf 等）導入、Worker（pdf.worker）を静的エクスポートで配信できるよう設定
- [ ] ファイル読込: D&Dゾーン + ファイル選択（初期の空状態UI）
- [ ] PDFドキュメントのロード・解析（ページ数・メタ情報取得）
- [ ] メインビューア: 選択ページの描画、ページ送り（◀ N/Total ▶）
- [ ] ズーム（スライダー＋％入力＋幅合わせ/全体表示）
- [ ] 連続スクロール / 単ページ送りの切替
- [ ] 左ペイン: サムネイル一覧の描画（低解像度・遅延生成）
- [ ] 左ペイン `Tabs`（サムネイル / しおり）の枠組み
- [ ] ステータスバー（ページ数・ファイルサイズ・処理状況）
- [ ] **テスト**: PDFロードでページ数が正しく取得される / ページ送り境界（先頭/末尾）/ ズーム値クランプ
- [ ] **計測**: 中規模PDF（数百ページ/数十MB）の初回描画時間・メモリ
