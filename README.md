...
- **文字の書き換え**: PDF 内の数字・文言を、元の文字データそのものを書き換えて変更（元の文字は PDF に残らない）。元の書体に無い文字は、同じ書体の別フォント → 同梱のゴシック体（Noto Sans JP）の順で補い、書体が変わる場合は確定前に表示
- **テーマ**: ライト / ダーク / システム追従
...
- **文字の書き換え**は同じ行の中の置き換えのみ（段落の折り返しはしない）。スキャン PDF・アウトライン化された文字・縦書き・Type3 フォント・Form XObject 内の文字は書き換えられない。同梱フォントはゴシック体 1 書体のみ。
- 同梱フォント（5.5MB）は、元の書体に無い文字を書き換えるときと、書き換えを含む PDF を保存するときに読み込む。
